...
ADMIN_ID = int(os.getenv("YOUR_TELEGRAM_ID"))
...
async def run_outreach(event):
    ...
    await event.respond(f"🚀 Outreach started! Sending to list starting at index: {idx}")

    for session in SESSIONS:
        if idx >= len(targets): break
        
        await event.respond(f"🔌 Connecting to: {session}...")
        try:
            async with TelegramClient(session, API_ID, API_HASH) as client:
                while idx < len(targets):
                    ...
                    try:
                        ...
                        await event.respond(f"⚠️ {session} hit limit. Switching account...")
                        break
                    except Exception:
                        idx += 1
                        continue
        except Exception as e:
            await event.respond(f"❌ Could not connect to {session}: {e}")
            continue
    
    await event.respond(f"✅ Batch complete. Final Progress: {idx} users messaged.")

@bot.on(events.NewMessage(pattern='/start', from_users=ADMIN_ID))
async def start(event):
    await event.respond("👋 Hoopstreet Manager Online.\n\n/send_now - Start sending\n/status - Check progress\n/reset - Reset to index 0")
...
async def status(event):
    ...
    await event.respond(f"📊 Current Progress: Index {idx} in targets.txt")

@bot.on(events.NewMessage(pattern='/reset', from_users=ADMIN_ID))
async def reset(event):
    with open('last_sent.txt', 'w') as f: f.write("0")
    await event.respond("🔄 Progress reset to 0.")
