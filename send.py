...
async def global_worker():
    while True:
        ...
        sched_row = supabase.table("message_campaign").select("*").eq("status", "scheduled").execute().data
        if sets['is_sched_active'] and sched_row:
            target_time = datetime.fromisoformat(sched_row[0]['updated_at'].replace('Z', '+00:00'))
            if datetime.now(pytz.utc) < target_time:
                await asyncio.sleep(60); continue
...
async def status(event):
    ...
    sched_info = next((x['updated_at'] for x in leads if x['status'] == 'scheduled'), "N/A")
    
    msg = (f"📊 **Global Audit: {datetime.now(PHT).strftime('%Y-%m-%d %H:%M')}**\n"
           f"📂 List: {len(leads)} | ⏳ Pend: {pending}\n"
           f"✅ Daily: {daily} | 📱 Accs: {len(glob.glob('*.session'))-1}\n"
           f"🛠️ Sched: {'ON' if sets['is_sched_active'] else 'OFF'} | 📅 Next: {sched_info}\n"
...
async def toggle_schedule(event):
    ...
    if sets['is_sched_active']:
        ...
        await event.respond("⏸️ **Schedule Cleared and Stopped.**")
    else:
        async with bot.conversation(event.sender_id) as conv:
            await conv.send_message("📅 **Enter Schedule (e.g., 2026-04-25 02:00 PM):**")
